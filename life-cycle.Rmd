...
We can now quit the container and re-enter
...
We can create a new image based on the changes we made
...
```{bash}
docker run docker-test/samtools samtools
```
...
The file can be imported with `docker import`. However, the true docker-way is to create a file (a ***Dockefile***) from which your image can be automatically re-built.