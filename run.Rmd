...
If you look in the example directory you should find a `test.bcf` output file. You can view this in VCF format using the `bcftools` utility available as part of SAMtools. `delly` manages to call a known tandem duplication in this example dataset, but more importantly, we've managed to install and run the tool in a relatively painless manner.
...
docker run -v `pwd`/example/:/data dellytools/delly samtools view -H /data/test.bam
```

```{bash, eval=FALSE}
docker run -v `pwd`/example/:/data dellytools/delly bcftools view /data/test.bcf
```

Later we will see how we can build our own container that includes a particular version of `samtools`.

Exercise: run `bcftools` to view the contents of the `test.bcf` file output by `delly`.
