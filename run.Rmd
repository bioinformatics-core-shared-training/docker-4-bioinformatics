...
As an example, the author of `delly` (a tool for calling structural variants) has created a docker container that we can run. Typically docker containers are uploaded to the central `Dockerhub` repository and submitted under a particular username (`trausch` in this case). It is common for a docker file to be under version control in github. 

- [delly dockerhub entry](https://hub.docker.com/r/trausch/delly/)
...
We can mount *volumes* from a particular location on our host drive onto the file system used in the docker container. Let's mount the `example/` directory of the course materials folder. Currently, this contains an [example `.bam` file](https://github.com/bioinformatics-core-shared-training/docker-4-bioinformatics/raw/master/example/bwa.sorted.bam) (mapped reads for a small region on chromosome 17) and [index](https://github.com/bioinformatics-core-shared-training/docker-4-bioinformatics/raw/master/example/bwa.sorted.bam.bai). The analysis we are going to do will also require a reference genome, which you can download from UCSC. 
...
Building such an image is fairly painless, as one can extend the existing Bioconductor image to include your own packages and data. We will explore this in the *Creating containers* section.
