...
```{bash}
docker run -v `pwd`/example/:/data dellytools/delly delly call -t DEL -o /data/test.bcf -g /data/chr17.fa /data/bwa.sorted.bam
```
...
```{bash}
docker run -v `pwd`/example/:/data dellytools/delly samtools --version
docker run -v `pwd`/example/:/data dellytools/delly samtools view -H /data/bwa.sorted.bam
```

Later we will see how we can build our own container that includes a particular version of `samtools`.
